...
case class InsertIntoCassandraTable(cassandraRelation: CassandraRelation,
  ...
  private lazy val result: RDD[Row] = {
    ...
    val writeConf = cc.getWriteConf(cassandraRelation.keyspaceName,
      cassandraRelation.tableName,
      cassandraRelation.cluster)

    childRdd.saveToCassandra(cassandraRelation.keyspaceName,
      cassandraRelation.tableName,
      AllColumns,
      writeConf)(
        new CassandraConnector(cc.getCassandraConnConf(cassandraRelation.cluster)),
