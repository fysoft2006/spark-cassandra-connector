...
package object cassandra {
  ...
  implicit class CSQLContext(sqlContext: SQLContext) {
    ...
    def cassandraTable(table: String,
      ...
      sqlContext.baseRelationToDataFrame(getDataSourceRelation(table,
        keyspace, scanType, cluster, userSpecifiedSchema))
    ...
    def getDataSourceRelation(table: String,
                              ...
                              userSpecifiedSchema: Option[StructType] = None) = {
      scanType match {
        case BaseScanType => CSTableScanRelation(table, keyspace, cluster, userSpecifiedSchema, sqlContext)
    ...
    def addTableReadConf(keyspace: String,
      ...
      ConfCache.addTableReadConf(keyspace, table, conf, cluster)
